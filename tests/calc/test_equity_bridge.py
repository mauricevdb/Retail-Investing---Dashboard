...
from dashboard.calc.equity_bridge import resolve
...
def test_equity_bridge_fallback_chain() -> None:
    end = date(2023, 12, 31)
    t = date(2024, 3, 1)

    # Primaire : Alpha porte StockholdersEquity.
    alpha_value, alpha_tag = resolve(_facts("0000000001"), cik="0000000001", end=end, t=t)
    assert alpha_value == 250000000
    assert alpha_tag == "us-gaap:StockholdersEquity"

    # Repli : Gamma n'a pas le tag primaire, seulement la variante incluant
    # les intérêts minoritaires.
    gamma_value, gamma_tag = resolve(_facts("0000000003"), cik="0000000003", end=end, t=t)
    assert gamma_value == 30000000
    assert gamma_tag == (
